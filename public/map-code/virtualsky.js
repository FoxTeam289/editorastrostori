...
@*/
	...
	@*/
		...
		this.colours = {
			normal: {
				...
				constellation: 'rgba(255,255,255,1)',
	...
	VirtualSky.prototype.createSky = function () {
		...
		if (this.c && this.c.getContext) {
			...
			var contextMenuHandler = !this.callback.contextmenu
				? undefined
				: {
	...
	VirtualSky.prototype.toggleHelp = function () {
		...
		if (S('.' + v + '_dismiss').length > 0) S('.' + v + '_dismiss').trigger('click');
			...
			for (i = 0; i < this.keys.length; i++) {
				if (this.keys[i].txt)
					S('.' + v + '_' + this.keys[i].txt).on('click', { fn: this.keys[i].fn, me: this }, function (e) {
						e.preventDefault();
	...
	VirtualSky.prototype.whichPointer = function (x, y) {
		for (var i = 0; i < this.pointers.length; i++) if (Math.abs(x - this.pointers[i].x) < 5 && Math.abs(y - this.pointers[i].y) < 5) return i;
	...
	VirtualSky.prototype.toggleInfoBox = function (i) {
		...
		if (S('#' + this.id + '_' + this.infobox).length <= 0) this.container.append('<div id="' + this.id + '_' + this.infobox + '" class="' + this.infobox + '" style="display:none;"></div>');
	...
	VirtualSky.prototype.drawImmediate = function (proj) {
		...
		this.startClip().drawGridlines('az').drawGridlines('eq').drawGridlines('gal').drawGalaxy().drawConstellationLines().drawConstellationBoundaries().drawStars().drawEcliptic().drawMeridian().drawPlanets().drawMeteorShowers().endClip().drawCardinalPoints();
					...
					if (S(hid + '_calendar').length == 0) {
						...
						s.container.append(
							'<div id="' +
								...
								id +
								'_date" value="' +
								(s.clock.getFullYear() + '-' + ((s.clock.getMonth() < 9 ? '0' : '') + (s.clock.getMonth() + 1)) + '-' + (s.clock.getDate() < 10 ? '0' : '') + s.clock.getDate()) +
								'" />' +
								'<input type="time" id="' +
								id +
								'_time" value="' +
								(s.clock.getHours() < 10 ? '0' : '') +
								s.clock.getHours() +
								':' +
								(s.clock.getMinutes() < 10 ? '0' : '') +
								s.clock.getMinutes() +
								'" />' +
								'</div>' +
								'</div>'
	...
	VirtualSky.prototype.createLightbox = function (lb, opts) {
		...
		function Lightbox(lb, vs, opts) {
			...
			this.bg = bg;
			this.vs.container.find('.virtualsky_dismiss').on('click', { lb: this }, function (e) {
				e.data.lb.close();
			});
		...
		Lightbox.prototype.resize = function () {
			function columize(wide, tall) {
				...
				this.lb
					...
					.css({ width: '100%' })
					.parent()
					.css({ width: w <= 500 ? '100%' : Math.min(w - bar, (mx + bar / 2) * n + bar) + 'px' });
	...
	VirtualSky.prototype.trigger = function (ev, args) {
		...
		var o = [];
		var _obj = this;
		if (typeof this.events[ev] === 'object') for (i = 0; i < this.events[ev].length; i++) if (typeof this.events[ev][i] === 'function') o.push(this.events[ev][i].call(_obj, args));
